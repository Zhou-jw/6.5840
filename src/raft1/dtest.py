...
def run_test(test: str, race: bool, timing: bool, timeout: int):
    test_cmd = ["go", "test", f"-timeout={timeout}s", f"-run={test}"]
    if race:
...
def run_tests(
    ...
    timeout: int           = typer.Option(60,     '--timeout',         '-d',    help='Timeout in seconds'),
    # fmt: on
):
    ...
    while True:
        ...
        total_task = total_progress.add_task(
            "[yellow]Tests[/yellow]", total=total)
        ...
        tasks = {test: task_progress.add_task(
            test, total=iterations) for test in tests}
                    ...
                    if n < workers:
                        for test in itertools.islice(test_instances, workers-n):
                            futures.append(executor.submit(
                                run_test, test, race, timing, timeout))
                    ...
                    for future in done:
                        ...
                        if rc != 0:
                            ...
                            if results[test]['completed'].n == iterations and results[test]['failed'].n == 0:
                                task_progress.update(
                                    tasks[test], description=f"[green]{test}[/green]")
